Feature: Send a unvote transaction


  Scenario: Successfully send an unvote transaction w/ Single Passphrase
    ...
    And the user has selected a delegate to unvote 
    When the user select 'Unvote' 
    ...
    And the user will see a 'Unvoted Succesfully' message
    And the delegate that the user voted for will have the 'Vote' label
    removed 

  Scenario: Successfully send an unvote transaction w/ Second Passphrase
    ...
    Then the transaction will be broadcast to the network
    And the user will see a 'Unvoted Succesfully' message
    And the delegate that the user voted for will have the 'Unvote' label
    removed 

  Scenario: Successfully send an unvote transaction w/ Encrypted Passphrase
    Given the user has selected a wallet with a second passphrase
    And the wallet has a balance 
    And the user has selects the 'Delegates' tab
    And the user has selected a delegate to unvote 
